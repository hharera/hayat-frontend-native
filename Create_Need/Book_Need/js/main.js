...
const isValidAuthor = author => {
    const correctAuthor = /^$|^[a-zA-Z0-9أ-ي\s-,_/]+$/
...
const isValidBookPublisher = publisher => {
    const correctPublisher = /^$|^[a-zA-Z0-9أ-ي\s-,_/]+$/
...
async function SendUserData(){
    Data=await fetch('http://146.190.206.136:8080/api/v1/needs/book', {
        method:"POST",
        headers:{
      ...
      .then(data => {
        console.log(data);
        url=`http://146.190.206.136:8080/api/v1/needs/book/${data.id}/images`;
